...
* **Save:** Creates a `.nfo` file which can be opened in `msinfo32` later but is fairly difficult to read manually.
* **Export:** Creates a `.txt` file which can't be opened in `msinfo32` but is easier to read manually.
...
## Dump files
***Main Article(s):*** [**BSOD Guide**](docs/learning/bsod_guide), [**Obtaining BSOD Dumps**](docs/factoids/bsod)
...
{: .info .info-icon }
>**Tools for analyzing dump files:**
> - [**WinDbg**](https://aka.ms/windbg/download): Microsoft's official tool for analyzing and [obtaining detailed information from dump (`.dmp`) files](docs/learning/bsod_guide) generated by BSODs or program crashes, such as the registers and stack trace.
> - **Other Tools**: Third-party tools like [**BlueScreenView**](https://www.nirsoft.net/utils/blue_screen_view.html) and [**WhoCrashed**](https://www.resplendence.com/download/whocrashedSetup.exe) can be used to generate quick summaries of dump files, but they are often highly inaccurate and are not recommended for detailed dump analysis.
...
{: .info .info-icon }
> **Note regarding pre-built computers:**
> 
> Some pre-builts use generic OEM power supplies without a clear model name or label. In these cases it's best to refer to the product page on the manufacturer's/seller's website to identify the power supply model used.
